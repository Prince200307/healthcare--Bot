...
class HealthcareChatbot:
    ...
    def analyze_symptoms(self):
        ...
        # Get disease info
        self.diagnosis_text.insert(END, f"🔍 Possible Condition: ", "bold")
        self.diagnosis_text.insert(END, f"{disease}\n\n", "disease")
        
        # Get typical symptoms for this disease
        red_cols = self.dimensionality_reduction.columns 
        typical_symptoms = red_cols[self.dimensionality_reduction.loc[disease].values[0].nonzero()]
        
        # Display results
        self.diagnosis_text.insert(END, "✅ Your Reported Symptoms:\n", "bold")
        for symptom in self.user_symptoms:
            self.diagnosis_text.insert(END, f"- {symptom}\n")
        
        self.diagnosis_text.insert(END, "\nℹ️ Typical Symptoms for This Condition:\n", "bold")
        for symptom in typical_symptoms:
            self.diagnosis_text.insert(END, f"- {symptom}\n")
        
        # Confidence level
        confidence = len(set(self.user_symptoms) & set(typical_symptoms)) / len(typical_symptoms)
        self.diagnosis_text.insert(END, f"\n📊 Confidence Level: ", "bold")
        self.diagnosis_text.insert(END, f"{confidence:.1%}\n\n", "confidence")
        
        # Doctor recommendation
        doctor = self.doctors[self.doctors['disease'] == disease]
        if not doctor.empty:
            self.diagnosis_text.insert(END, "👨‍⚕️ Recommended Specialist:\n", "bold")
            self.diagnosis_text.insert(END, f"Dr. {doctor['Name'].values[0]}\n\n", "doctor")
            
            self.diagnosis_text.insert(END, "🔗 More Information:\n", "bold")
            hyperlink = HyperlinkManager(self.diagnosis_text)
            def open_link():
                webbrowser.open_new(doctor['Description'].values[0])
            self.diagnosis_text.insert(INSERT, doctor['Description'].values[0], 
                                      hyperlink.add(open_link))
        
        # Configure text tags for styling
        self.diagnosis_text.tag_config("bold", font=('Helvetica', 10, 'bold'))
        self.diagnosis_text.tag_config("disease", foreground=ACCENT, 
                                      font=('Helvetica', 10, 'bold'))
        self.diagnosis_text.tag_config("confidence", foreground=DARK_BLUE)
        self.diagnosis_text.tag_config("doctor", foreground=DARK_BLUE)
...
class HealthcareChatbot:
    ...
    def analyze_symptoms(self):
        ...
        # Get disease info
        self.diagnosis_text.insert(END, "######## DIAGNOSIS RESULTS ########\n\n", "header")
        self.diagnosis_text.insert(END, f"Possible condition: {disease}\n\n", "bold")
        
        # Get typical symptoms for this disease
        red_cols = self.dimensionality_reduction.columns 
        symptoms_given = red_cols[self.dimensionality_reduction.loc[disease].values[0].nonzero()]
        
        # Display results (matching original console output format)
        self.diagnosis_text.insert(END, "Symptoms you reported:\n", "bold")
        for symptom in self.user_symptoms:
            self.diagnosis_text.insert(END, f"- {symptom}\n")
        self.diagnosis_text.insert(END, "\n")
        
        self.diagnosis_text.insert(END, "Typical symptoms for this condition:\n", "bold")
        for symptom in symptoms_given:
            self.diagnosis_text.insert(END, f"- {symptom}\n")
        self.diagnosis_text.insert(END, "\n")
        
        # Confidence level
        confidence = len(set(self.user_symptoms) & set(symptoms_given)) / len(symptoms_given)
        self.diagnosis_text.insert(END, f"Confidence level: {confidence:.1%}\n\n", "bold")
        
        # Doctor recommendation
        doctor = self.doctors[self.doctors['disease'] == disease]
        if not doctor.empty:
            self.diagnosis_text.insert(END, "The model suggests:\n", "bold")
            self.diagnosis_text.insert(END, f"Consult Dr. {doctor['Name'].values[0]}\n")
            self.diagnosis_text.insert(END, f"Visit: ", "bold")
        ...
        # Configure text tags for styling
        self.diagnosis_text.tag_config("header", font=('Helvetica', 12, 'bold'), 
                                     foreground=DARK_BLUE, justify='center')
        self.diagnosis_text.tag_config("bold", font=('Helvetica', 10, 'bold'))
    ...
    def provide_diagnosis(self):
        ...
        present_disease = self.labelencoder.inverse_transform(
            np.nonzero(self.classifier.tree_.value[self.current_node])[1])[0]
        
        self.response_text.delete(1.0, END)
        self.response_text.insert(END, "######## DIAGNOSIS RESULTS ########\n\n", "header")
        self.response_text.insert(END, f"You may have: {present_disease}\n\n", "bold")
        
        # Get symptoms
        red_cols = self.dimensionality_reduction.columns 
        symptoms_given = red_cols[self.dimensionality_reduction.loc[present_disease].values[0].nonzero()]
        
        self.response_text.insert(END, "Symptoms you reported:\n", "bold")
        for symptom in self.symptoms_present:
            self.response_text.insert(END, f"- {symptom}\n")
        self.response_text.insert(END, "\n")
        
        self.response_text.insert(END, "Typical symptoms for this condition:\n", "bold")
        for symptom in symptoms_given:
            self.response_text.insert(END, f"- {symptom}\n")
        self.response_text.insert(END, "\n")
        
        # Confidence level
        confidence = len(self.symptoms_present)/len(symptoms_given)
        self.response_text.insert(END, f"Confidence level: {confidence:.1%}\n\n", "bold")
        
        # Doctor recommendation
        doctor = self.doctors[self.doctors['disease'] == present_disease]
        if not doctor.empty:
            self.response_text.insert(END, "The model suggests:\n", "bold")
            self.response_text.insert(END, f"Consult Dr. {doctor['Name'].values[0]}\n")
            self.response_text.insert(END, f"Visit: ", "bold")
            
            hyperlink = HyperlinkManager(self.response_text)
            def open_link():
                webbrowser.open_new(doctor['Description'].values[0])
            self.response_text.insert(INSERT, doctor['Description'].values[0], 
                                    hyperlink.add(open_link))
        
        # Configure text tags
        self.response_text.tag_config("header", font=('Helvetica', 12, 'bold'), 
                                    foreground=DARK_BLUE, justify='center')
        self.response_text.tag_config("bold", font=('Helvetica', 10, 'bold'))
